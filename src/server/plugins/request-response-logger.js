      ...
      server.ext('onPreResponse', (request, h) => {
        ...
        if (pluginSetting.logging) {
          ...
          if (isPrettyLoggingEnabled) {
            ...
            const customMessage = `[HTTP Response] ${url} [${request.method.toUpperCase()}][${statusCode}]`;

            if (statusCode < 400) {
              logger.info(customMessage, output, { $logOptions: { depth: 2, customMessage } });
            }

            else if (statusCode >= 400 && statusCode < 500) {
              logger.warn(customMessage, output, { $logOptions: { depth: null, customMessage } });
            }

            else {
              logger.error(customMessage, output, { $logOptions: { depth: null, customMessage } });
            }
          }
          else {
