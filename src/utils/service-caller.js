  ...
  async request({
    ...
    if (isPrettyLoggingEnabled) {
      ...
      logger.debug(title, {
        ...
        data,
        timeout,
      }, { $logOptions: { depth: 2, customMessage: title } });
    }
      ...
      .then((response) => {
        ...
        if (isPrettyLoggingEnabled) {
          ...
          if (response.status < 400) {
            logger.info(title, response.data, { $logOptions: { depth: 2, customMessage: title } });
          }
          else if (response.status >= 400 && response.status < 500) {
            logger.warn(title, response.data, { $logOptions: { depth: 2, customMessage: title } });
          }
          else {
            logger.error(title, response.data, { $logOptions: { depth: 2, customMessage: title } });
          }
        }
        else {
